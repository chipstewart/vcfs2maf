...
isna(x::AbstractString)=false
isna(x::Number)=isnan(x)
isna(x::Missing)=true
...
#   end
    ...
    if isdefined(:df)
       ...
       fx=setdiff(f1,f2)
       for f in fx
           df2[f]=fill("",size(df2[:chr]))
       end
       fx=setdiff(f2,f1)
       for f in fx
           df[f]=fill("",size(df[:chr]))
       end
