...
# ARGS=["THCA-EM-A3FL-TP", "THCA-EM-A3FL-NB",  "/opt/test/THCA-EM-A3FL-TP-NB.strelka2_snv.tsv", "/opt/test/THCA-EM-A3FL-TP-NB.strelka2_indel.tsv", "/opt/test/THCA-EM-A3FL-TP-NB.strelka2.maflite.tsv"]
...
#Strelka SNV vcf
df = readtable(file1)
print("input SNV size: ",size(df),"\n")
df = df[df[:FILTER] .== "PASS" ,:]
print("FILTER=PASS SNV size: ",size(df),"\n")
...
#Strelka INDEL vcf
df = readtable(file2)
print("input INDEL size: ",size(df),"\n")
df = df[df[:FILTER] .== "PASS" ,:]
print("FILTER=PASS INDEL size: ",size(df),"\n")
...
if size(df)[1]>0
    ...
    # describe(df)
    # print(df)
    delete!(df, [:FILTER,:QUAL,:ID,:NT,:SOMATIC,:QSI_NT,:TQSI_NT,:SGT])
    # head(df)
...
# maflite fields 
# SNP: build	chr	start	end	ref_allele	alt_allele	tumor_barcode	normal_barcode	tumor_f	init_t_lod	t_lod_fstar	t_alt_count	t_ref_count	judgement
# INDEL: build	contig	start_position	end_position	ref_allele	alt_allele	tumor_name	normal_name	tumor_f	n_ref_count	n_alt_count	t_ref_count	t_alt_count	init_n_lod	init_t_lod	judgement
